...
const ChipFilters = (): JSX.Element => {
    ...
    return (
        <div>
            {chips.map((chip) => (<Chip key={chip.label} classes={{root: classnames(chip.className, classes.chip, classes.selected && isSelected===chip.label?classes.selected:"")}} label={chip.label} clickable onClick={resolveClick}/>))}
        </div>
    )
}
